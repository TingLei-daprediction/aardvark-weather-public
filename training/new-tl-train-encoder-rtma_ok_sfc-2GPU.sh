#!/bin/bash
# 2-GPU single-node RTMA-OK encoder run: effective global batch 2 at batch-1 per-GPU
# memory. --batch_size is PER PROCESS, so it stays 1; DDP averages gradients across the
# 2 ranks (mp.spawn world_size auto-detects 2 GPUs). NOTE: must stay single-node --
# ddp_setup hardcodes MASTER_ADDR=localhost (train_module.py), so multi-node ranks
# cannot rendezvous through this launch path.
#SBATCH -A fv3-cam
#SBATCH -J av-ok-sfc-2gpu
#SBATCH -p u1-h100
#SBATCH -q gpuwf
#SBATCH -N 1
...
output_dir="/scratch3/NCEPDEV/fv3-cam/Ting.Lei/aardvark-data/dr-rtma/OK-output-2gpu/"
