#!/bin/bash
# Warm-start a further 300 epochs from the latest best checkpoint written by the
# first warmstart run (which itself warm-started from the original lr=5e-5 run;
# loss dropped 0.11 -> 0.07 there and was still decreasing). Model weights only
# are restored: optimizer state and displayed epoch numbering restart from zero.
# Outputs go to a new directory so both prior runs are preserved.
#SBATCH -A fv3-cam
#SBATCH -J av-ok-sfc-2gpu-warm2
#SBATCH -p u1-h100
#SBATCH -q gpuwf
#SBATCH -N 1
...
#SBATCH -t 24:00:00
#SBATCH -o new-aardvark-gpu-train-ok-2gpu-warmstart2.%j.out
#SBATCH -e new-aardvark-gpu-train-ok-2gpu-warmstart2.%j.err
...
source_output_dir="/scratch3/NCEPDEV/fv3-cam/Ting.Lei/aardvark-data/dr-rtma/OK-output-2gpu-lr5e-5-warmstart/"
output_dir="/scratch3/NCEPDEV/fv3-cam/Ting.Lei/aardvark-data/dr-rtma/OK-output-2gpu-lr5e-5-warmstart2/"
