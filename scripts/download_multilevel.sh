...
# Pressure levels (hPa) needed for Aardvark 4u/13u modes; adjust as needed
levels = [
    "1000","925","850","700","600","500","400","300","250","200","150","100","70",
    "50","30","20","10","7","5","3","2","1"
]

# Variables to retrieve on these levels
variables = [
    "geopotential",
    "temperature",
    "specific_humidity",
...
for year in years:
    for month in months:
        target = f"pressure_level/era5_pl_1p5deg_{year}_{month}.nc"
        if os.path.exists(target):
            print(f"Skip {target}, exists")
            continue
        print(f"Request {year}-{month}")
        c.retrieve(
